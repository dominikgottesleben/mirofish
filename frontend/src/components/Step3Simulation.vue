    ...
    <div class="control-bar">
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          ...
          <div class="platform-stats">
            ...
            <span class="stat">
              <span class="stat-label">Vergangene Zeit</span>
          ...
          <!-- Verfügbare Aktionen Tooltip -->
          <div class="actions-tooltip">
            <div class="tooltip-title">Verfügbare Aktionen</div>
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Wird gestartet...' : 'Ergebnisbericht generieren' }}
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div class="timeline-header" v-if="allActions.length > 0">
        <div class="timeline-stats">
          <span class="total-count">EREIGNISSE GESAMT: <span class="mono">{{ allActions.length }}</span></span>
      ...
      <div class="timeline-feed">
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">Repostet von @{{ action.action_args?.original_author_name || 'User' }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">Hat den Beitrag von @{{ action.action_args?.post_author_name || 'User' }} geliked</span>
                  </div>
                ...
                <template v-if="action.action_type === 'CREATE_COMMENT'">
                  ...
                  <div v-if="action.action_args?.post_id" class="comment-context">
                    ...
                    <span>Antwort auf Beitrag #{{ action.action_args.post_id }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'FOLLOW'">
                  <div class="follow-info">
                    ...
                    <span class="follow-label">Folgt jetzt @{{ action.action_args?.target_user || action.action_args?.user_id || 'User' }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'UPVOTE_POST' || action.action_type === 'DOWNVOTE_POST'">
                  <div class="vote-info">
                    ...
                    <span class="vote-label">{{ action.action_type === 'UPVOTE_POST' ? 'Hochgestimmt' : 'Runtergestimmt' }}</span>
                  </div>
                ...
                <!-- DO_NOTHING: Keine Aktion (Leerlauf) -->
                <template v-if="action.action_type === 'DO_NOTHING'">
                  <div class="idle-info">
                    <svg class="icon-small" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
                    <span class="idle-label">Aktion übersprungen</span>
                  </div>
                </template>

                <!-- Allgemeiner Fallback: Unbekannter Typ oder Content, der oben nicht behandelt wurde -->
              ...
              <div class="card-footer">
                ...
                <!-- Plattform-Tag entfernt, da es jetzt im Header ist -->
              </div>
            </div>
          </div>
        </TransitionGroup>
        ...
        <div v-if="allActions.length === 0" class="waiting-state">
          ...
          <span>Warten auf Agenten-Aktionen...</span>
        </div>
      </div>
    </div>

    <!-- Untere Info / Protokolle -->
    <div class="system-logs">
      <div class="log-header">
        <span class="log-title">SIMULATIONSMONITOR</span>
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Maximale Rundenanzahl aus Step2
  minutesPerRound: {
    type: Number,
    default: 30 // Standard: 30 Minuten pro Runde
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
// Computed
// Aktionen in chronologischer Reihenfolge anzeigen (neueste unten)
const chronologicalActions = computed(() => {
  return allActions.value
})

// Aktionszähler pro Plattform
...
// Simulierte vergangene Zeit formatieren (basierend auf Runden und Minuten pro Runde)
...
// Simulierte vergangene Zeit der Twitter-Plattform
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Simulierte vergangene Zeit der Reddit-Plattform
...
// Alle Zustände zurücksetzen (für Neustart der Simulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
const doStartSimulation = async () => {
  if (!props.simulationId) {
    addLog('Fehler: simulationId fehlt')
    return
  }

  // Alle Zustände zurücksetzen, um Einflüsse der vorherigen Simulation zu vermeiden
  resetAllState()

  isStarting.value = true
  startError.value = null
  addLog('Parallele Dual-Plattform-Simulation wird gestartet...')
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(`Maximale Simulationsrunden gesetzt: ${props.maxRounds}`)
    }
    
    addLog('Dynamischer Graph-Aktualisierungsmodus aktiviert')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Alte Simulationsprotokolle bereinigt, Simulation wird neu gestartet')
      }
      addLog('✓ Simulationsengine erfolgreich gestartet')
      ...
      addLog(`✗ Start fehlgeschlagen: ${res.error || 'Unbekannter Fehler'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ Startausnahme: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  addLog('Simulation wird gestoppt...')
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`Stoppen fehlgeschlagen: ${res.error || 'Unbekannter Fehler'}`)
    }
  } catch (err) {
    addLog(`Stoppausnahme: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
// Vorherige Runde pro Plattform verfolgen, um Änderungen zu erkennen und zu protokollieren
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Rundenänderungen pro Plattform erkennen und protokollieren
    ...
    console.warn('Laufzeitstatus konnte nicht abgerufen werden:', err)
  }
}

// Prüfen, ob alle aktivierten Plattformen abgeschlossen sind
const checkPlatformsCompleted = (data) => {
  // Wenn keine Plattformdaten vorhanden, false zurückgeben
  if (!data) return false

  // Abschlussstatus jeder Plattform prüfen
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true

  // Prüfen, ob alle aktivierten Plattformen abgeschlossen sind
  // Plattform gilt als aktiviert, wenn count > 0 oder running einmal true war
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted

  // Wenn keine Plattform aktiviert ist, false zurückgeben
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Neue Aktionen inkrementell hinzufügen (mit Deduplizierung)
      ...
      // Kein automatisches Scrollen, damit der Benutzer die Zeitleiste frei durchsehen kann
      // Neue Aktionen werden unten angehängt
    }
  } catch (err) {
    console.warn('Detailstatus konnte nicht abgerufen werden:', err)
  }
}
...
const formatActionTime = (timestamp) => {
  if (!timestamp) return ''
  try {
    return new Date(timestamp).toLocaleTimeString('de-DE', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' })
  } catch {
    return ''
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Berichtgenerierungsanfrage gesendet, bitte warten...')
    return
  }

  isGeneratingReport.value = true
  addLog('Berichterstellung wird gestartet...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Berichterstellungsaufgabe gestartet: ${reportId}`)

      // Zur Berichtsseite navigieren
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ Start der Berichterstellung fehlgeschlagen: ${res.error || 'Unbekannter Fehler'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ Ausnahme beim Starten der Berichterstellung: ${err.message}`)
    isGeneratingReport.value = false
  }
}

// Protokoll nach unten scrollen
...
onMounted(() => {
  addLog('Step3 Simulationslauf initialisiert')
  if (props.simulationId) {
    doStartSimulation()
  }
})
